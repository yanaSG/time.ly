...
const Dashboard: React.FC = () => {
  const { notebooks, getNotebookById } = useNotebooks();
  ...
  useEffect(() => {
    ...
    const trackActivity = () => {
      ...
      if (elapsed < INACTIVITY_THRESHOLD) {
        setActiveTime(prev => prev + (elapsed / 1000));
      }
      setLastActivity(now);
    };
  ...
  }, [activeTime, lastActivity, user, updateUserActivity]);
  ...
  const debouncedSaveSchedule = useCallback(
    debounce(async (content: string) => {
      ...
      try {
        await updatePostItNote(content);
  ...
  const handleSaveSchedule = () => {
    debouncedSaveSchedule.cancel();
    debouncedSaveSchedule(scheduleContent);
